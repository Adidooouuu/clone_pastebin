<?php
  ...
  if (isset($_POST['submit']))
  {
    ...
    $url = "form_treated?s=$random_id";

    // ADD DATA TO DB ; USER IS LOGGED IN = ADD ITS ID & USERNAME, ELSE IT STAYS NULL
    if (@$_SESSION["Auth_OK"] == true)
    {
      $table_add_query = "INSERT INTO $links_table (title, content, random_id, user_name, random_id_user) VALUES (:link_name, :text_content, :random_id, :user_name, :user_random_id)";
      ...
      $insert_new_content->execute(
                                    [
                                      ...
                                      "user_random_id" => $_SESSION["user_random_id"],
                                    ]
                                  );
    }
    else
    {
      $table_add_query = "INSERT INTO $links_table (title, content, random_id) VALUES (:link_name, :text_content, :random_id)";
      $insert_new_content = $bdd_connection->prepare($table_add_query);
      $insert_new_content->execute(
                                    [
                                      "link_name" => $link_name,
